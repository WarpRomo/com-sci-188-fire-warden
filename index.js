  ...
  for(var i = 0; i < keys.length; i++){
    ...
    if(keys[i].startsWith("drone")){
      ...
      if(targetObj != undefined){
        ...
        if(dist > 140){
          currentPos = moveToPos(currentPos, targetPos, 10);

          //console.log("UPDATED CURRENT", currentPos);
          currentPos[0] = Math.floor(currentPos[0]);
          currentPos[1] = Math.floor(currentPos[1]);
    ...
    ws.on('message', message => {
        ...
        if(data.event == "simulationState"){
          ...
          if(data.state == "Reasoning"){
            ...
            const prompt = `"Position of drones: ${droneArrString}. Position of fires: ${fireArrString}. Position of objects: ${objectArrString}. Redirect each and every drone to target the fires efficiently. Output in the exact format (Example): JJJ [ { droneID: drone69, targetFire: fire32 }, { droneID: drone42, targetFire: fire47 }, ...all drones ] JJJ. Start and end your JSON output with the following characters: JJJ. Be sure JSON is valid with double quotes. Prioritize certain fires based on surroundings. You are not allowed to use tools such as code or others, only your reasoning is allowed. Explain your reasoning above the JSON."`;
